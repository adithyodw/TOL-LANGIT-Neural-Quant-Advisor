...
input double InpZScore_Level  = 1.0;  // Stat-Sig Breakout Level (0.5 - 2.0)
...
int OnInit() {
   ...
   if(hEMA == INVALID_HANDLE || hATR == INVALID_HANDLE || hADX == INVALID_HANDLE || hBB == INVALID_HANDLE || hRSI == INVALID_HANDLE) {
...
void OnTick() {
   ...
   if(CopyBuffer(hEMA,0,1,2,ema)<2 || CopyBuffer(hATR,0,1,2,atr)<2 || 
      ...
      CopyClose(_Symbol,_Period,1,2,close)<2) return;

   // 5. Quantitative Math: Z-Score
   // Calculation: (Close - Mean) / Standard Deviation
...
//+------------------------------------------------------------------+
void ExecuteTrade(int signal, double atr_val) {
   ...
   if(m_trade.PositionOpen(_Symbol, (signal == 1 ? ORDER_TYPE_BUY : ORDER_TYPE_SELL), lot, price, sl, tp)) {
      PrintFormat("Trade Opened: %s | Lot: %.2f | Z-Score: %.2f", (signal==1?"BUY":"SELL"), lot, 0.0);
   }
}
...
double CalculateLot(double sl_dist) {
   ...
   if(sl_dist <= 0 || tick_value <= 0) return 0;
...
int GetTradesToday() {
   ...
   for(int i = HistoryDealsTotal()-1; i >= 0; i--) {
      ...
      if(HistoryDealGetInteger(ticket, DEAL_MAGIC) == InpMagic && HistoryDealGetInteger(ticket, DEAL_ENTRY) == DEAL_ENTRY_IN) {
         count++;
      }
   }
   return count;
